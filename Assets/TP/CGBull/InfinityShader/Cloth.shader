...
////// Material Property
                #if _UseBaseColorTex
                    float3 BaseColor = tex2D(_BaseColorTexture, i.uv0 * _BaseColorTile) * _BaseColor * (1 - _Reflectance);
                    float3 SpecularColor = lerp(0.08 * _SpecularLevel, BaseColor, _Reflectance);
                #else
                    float3 BaseColor = _BaseColor * (1 - _Reflectance);
...
/////// Final Color
                ...
                half4 Preintegrated_DGF = PreintegratedDGF_LUT(_PreintegratedLUT, EnergyCompensation, SpecularColor, Roughness, BSDFContext.NoV);
                ...
                #if _UseSilk
                    float3 ClothShading = Cloth_Silk(BSDFContext, AnisoBSDFContext, Attenuation, EnergyCompensation, BaseColor, SpecularColor, Roughness, RoughnessT, RoughnessB);  
                    float3 GlobalIllumination = (BaseColor * gi.indirect.diffuse * Preintegrated_DGF.a) + (gi.indirect.specular * Preintegrated_DGF.rgb);
                #else
                    float3 ClothShading = Cloth_Cotton(BSDFContext, Attenuation, BaseColor, SpecularColor, Roughness);
                    ...
                    float3 GlobalIllumination = (BaseColor * gi.indirect.diffuse * Preintegrated_DGF.a) + (gi.indirect.specular * PreintegratedGF);  
                #endif
