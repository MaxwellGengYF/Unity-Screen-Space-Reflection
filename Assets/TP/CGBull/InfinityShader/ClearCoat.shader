Shader "CGBull/Infinity_Shader/ClearCoat" {
    Properties {
        ...
        _BaseColor ("BaseColor", Color) = (0, 0.75, 1, 1)
    ...
    SubShader {
        ...
        Pass {
            ...
            float _NormalTile, _ClearCoatNormalTile, _Iridescence_Distance, _SpecularLevel, _Roughness, _Reflectance, _ClearCoat, _ClearCoatRoughness;
            float4 _BaseColor, _TransmissionColor;
            sampler2D _PreintegratedLUT, _NomralTexture, _ClearCoatNomralTexture; 
...
////// Material Property
                float3 BaseColor = _BaseColor.rgb * (1 - _Reflectance);
                float Roughness = clamp(_Roughness, 0.04, 1);

                #if _Iridescence
                    float3 Iridescence_Color = Flim_Iridescence(1, BSDFContext.NoV, _Iridescence_Distance, 0.04);
                    float3 SpecularColor = lerp(0.08 * _SpecularLevel, Iridescence_Color, _Reflectance);
                #else
                    float3 SpecularColor = lerp(0.08 * _SpecularLevel, _BaseColor.rgb, _Reflectance);
                #endif
...
/////// Final Color
                ...
                float3 GlobalIllumination = (BaseColor * gi.indirect.diffuse * Preintegrated_DGF.a) + (lerp(gi.indirect.specular * Preintegrated_DGF.rgb * EnergyCompensation, ClearCoat_Reflection, ClearCoatIntensity));

                float3 ClearCoat_Shading = ClearCoat_Lit(BSDFContext, Attenuation, EnergyCompensation, ClearCoat_EnergyCompensation, BaseColor, SpecularColor, _ClearCoat, ClearCoatRoughness, Roughness);
