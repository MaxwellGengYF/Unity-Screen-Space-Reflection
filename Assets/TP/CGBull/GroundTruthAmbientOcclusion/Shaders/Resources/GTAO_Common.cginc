...
half4	_SSAO_UVToView, _SSAO_TexelSize, _SSAO_FadeValues;

half4x4	_SSAO_WorldToCameraMatrix, _SSAO_CameraToWorldMatrix, _SSAO_InverseViewProjectionMatrix;

sampler2D _SSAO_SceneColor_RT, _SSAO_Occlusion_RT, _SSAO_Spatial_RT, _SSAO_TemporalPrev_RT, _SSAO_TemporalCurr_RT, _CameraGBufferTexture0, _CameraGBufferTexture1, _CameraGBufferTexture2, _CameraReflectionsTexture, _CameraMotionVectorsTexture, _CameraDepthTexture;
...
half ReflectionOcclusion_LUT(half2 screenPos, half AO, half Roughness, half3 BN)
{
...
{
	float3 SSAOTexture = tex2Dlod(_SSAO_Occlusion_RT, float4(uv, 0.0, 0.0)).rgb;
	AO_RO = SSAOTexture.xy;
	//AO_Depth = SSAOTexture.z;
	AO_Depth = LinearEyeDepth(tex2Dlod(_CameraDepthTexture, float4(uv, 0, 0)).r);
}
